...
public class DataSet {
	...
	BufferedReader in;
	String line;
	List all_state_seq = new ArrayList();
	List all_output_seq = new ArrayList();
	Map state_map = new HashMap();
	Map output_map = new HashMap();
	...
	while (true) {
		...
		if (size > 0) {
			...
			DataItem d = (DataItem) item_list.get(i);
			state_seq[i] = d.state;
		...
		int cur_state = ((Integer) state_map.get(s)).intValue();
		...
		for (int i = 1; i < words.length; i++) {
		    ...
		    int cur_output = ((Integer) output_map.get(o)).intValue();
		...
		testState = (int [][]) all_state_seq.toArray(new int[0][]);
		testOutput = (int [][]) all_output_seq.toArray(new int[0][]);
		...
		trainState = (int [][]) all_state_seq.toArray(new int[0][]);
		trainOutput = (int [][]) all_output_seq.toArray(new int[0][]);
	...
	Iterator it = state_map.keySet().iterator();
	while (it.hasNext()) {
	    s = (String) it.next();
	    stateName[((Integer) state_map.get(s)).intValue()] = s;
	}
	...
	while (it.hasNext()) {
	    s = (String) it.next();
	    outputName[((Integer) output_map.get(s)).intValue()] = s;
	}
