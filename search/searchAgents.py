...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            # used these...
            # took sketch from below in food seach problem
            x, y = state[0]
	    dx, dy = Actions.directionToVector(action)
	    nextx, nexty = int(x + dx), int(y + dy)
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    #Worked with Jacob Rail - asked for assistance
    #manhatten distance!
    def manDist(nodeStart,nodeEnd):
		xy1 = nodeStart
		xy2 = nodeEnd
		return abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])

    currentNode,boolTup= state
    cornersLeft = 0
    manhattanDistance = 0
	
    for i in range(0,len(boolTup)):
	if not boolTup[i]:
            newManhattanDistance = manDist(currentNode,corners[i])
...
def foodHeuristic(state, problem):
    ...
    for f in foodGrid.asList():
        newDist = manDist(position,f)
	if newDist > dist:
            dist = newDist

    return dist
    '''
    Solution that I found that get 5/4. but not sure how it worked.
    x,y = position
    if foodGrid.count() == 0:
	return 0
